...
def handle_user_prompt_submit(data: dict) -> dict:
    ...
    # PreToolUse + Stop skill-first gates key off. Requirement (user,
    # 2026-07-02): plugin colon-namespaced commands are NOT exempt — Skill()
    # is required for all slash commands, so the gates must arm for them too.
    try:
